...
def sort(a, temp, low, up):
    ...
    # merge dictionary[low]...dictionary[mid] and dictionary[mid+1]...dictionary[up] to temp[low]...temp[up]
    merge(a, temp, low, mid, mid+1, up)
    # copy temp[low]....temp[up] to dictionary[low]...dictionary[up]
    copy(a, temp, low, up)
    return a
...
# copies temp[low]...temp[up] to dictionary[low]...dictionary[up]
...
def partition(a,low, up):
    ...
    while i <= j:
        ...
        while a[j] > pivot:
            j -= 1
        if i < j:  # swap dictionary[i] and dictionary[j]
            temp = a[i]
            a[i] = a[j]
            a[j] = temp
            i += 1
